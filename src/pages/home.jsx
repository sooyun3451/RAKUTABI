  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        checkInOutRef.current && !checkInOutRef.current.contains(event.target)
      ) {
                ...
                {currentMonthWeeks.map((week, weekIndex) => (
                  <div key={weekIndex} className="week">
                    {week.map((date, dayIndex) => (
                      <div
                        key={dayIndex}
                        className={`day ${date.getUTCMonth() !== month ? 'other-month' : ''} ${isCheckIn(date) ? 'selected check-in' : ''} ${isCheckOut(date) ? 'selected check-out' : ''} ${isSelected(date) ? 'in-range' : ''} ${date < today ? 'disabled' : ''}`}
                ...
                {nextMonthWeeks.map((week, weekIndex) => (
                  <div key={weekIndex} className="week">
                    {week.map((date, dayIndex) => (
                      <div
                        key={dayIndex}
                        className={`day ${date.getUTCMonth() !== month + 1 ? 'other-month' : ''} ${isCheckIn(date) ? 'selected check-in' : ''} ${isCheckOut(date) ? 'selected check-out' : ''} ${isSelected(date) ? 'in-range' : ''} ${date < today ? 'disabled' : ''}`}
                        onClick={() => handleDateClick(date)}
                      >
                        {date.getUTCDate()}
                      </div>
